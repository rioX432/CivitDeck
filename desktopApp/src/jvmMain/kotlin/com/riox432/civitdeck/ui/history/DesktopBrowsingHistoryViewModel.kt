...
data class DesktopDateGroup(
    val label: String,
    val items: List<RecentlyViewedModel>,
)

data class DesktopBrowsingHistoryUiState(
    val groups: List<DesktopDateGroup> = emptyList(),
    val isEmpty: Boolean = true,
)

class DesktopBrowsingHistoryViewModel(
    ...
    private val _uiState = MutableStateFlow(DesktopBrowsingHistoryUiState())
    val uiState: StateFlow<DesktopBrowsingHistoryUiState> = _uiState

    init {
        scope.launch {
            observeRecentlyViewedUseCase(limit = HISTORY_LIMIT).collect { items ->
                _uiState.update {
                    DesktopBrowsingHistoryUiState(
    ...
    fun deleteItem(historyId: Long) {
        scope.launch { deleteBrowsingHistoryItemUseCase(historyId) }
    }

    fun clearAll() {
        scope.launch { clearBrowsingHistoryUseCase() }
    }
...
private fun groupByDate(items: List<RecentlyViewedModel>): List<DesktopDateGroup> {
    ...
    return buildList {
        if (today.isNotEmpty()) add(DesktopDateGroup("Today", today))
        if (yesterday.isNotEmpty()) add(DesktopDateGroup("Yesterday", yesterday))
        if (thisWeek.isNotEmpty()) add(DesktopDateGroup("This Week", thisWeek))
        if (earlier.isNotEmpty()) add(DesktopDateGroup("Earlier", earlier))
    }
}
